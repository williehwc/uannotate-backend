...
// Solr lookup
app.get('/solr/:vocabulary/:query', function (req, res) {
  var requestURL = 'https://playground.phenotips.org/get/PhenoTips/SolrService?vocabulary=' + req.params.vocabulary + '&q=' + req.params.query;
  request({
      url: requestURL,
      json: true
...
app.post('/restricted/phenository/prof/diseases', function(req, res) {
  var filter = connection.escape(req.body.filter);
...
app.post('/restricted/annotation/view/full', function(req, res) {
  ...
  var query3 = 'SELECT phenotypes.id AS phenotypeID, hpo, observed, frequency, onset, not_ok, system AS systemHPO FROM phenotypes WHERE annotation_id = ' + annotationID + ' ORDER BY phenotypeID ASC;';
...
app.post('/restricted/annotation/prof/clone', function(req, res) {
  ...
  connection.query(query, function(err, result) {
    ...
    query = 'INSERT INTO phenotypes (annotation_id, hpo, observed, frequency, onset, prev_id) SELECT ' + newAnnotationID + ', hpo, observed, frequency, onset, id FROM phenotypes WHERE annotation_id = ' + annotationID;
    connection.query(query);
...
app.post('/restricted/annotation/system', function(req, res, next) {
  ...
  connection.query(query0 + query1, function(err, rows) {
    ...
    request({
      ...
      if (!error && response.statusCode == 200 && body.rows.length > 0) {
        for (var i = 0; i < body.rows[0].term_category.length; i++) {
          requestURL = 'https://playground.phenotips.org/get/PhenoTips/SolrService?vocabulary=hpo&q=' + body.rows[0].term_category[i];
          request({
              url: requestURL,
              json: true
            }, function (error, response, body) {
...
app.post('/restricted/annotation/compare', function(req, res) {
  ...
  connection.query(query0 + query1 + query2, function(err, rows) {
    ...
    var query3 = 'SELECT hpo, observed, frequency, onset, system AS systemHPO FROM phenotypes WHERE annotation_id = ' + annotationID + ';';
    var query4 = query3;
    if (rows[0].length > 0)
      query4 = 'SELECT hpo, observed, frequency, onset, system AS systemHPO FROM phenotypes WHERE annotation_id = ' + compareToAnnotationID + ';';
    var query5 = 'SELECT annotations.id AS annotationID, name AS diseaseName FROM annotations INNER JOIN diseases ON annotations.disease_id = diseases.id WHERE exercise_id IN (SELECT exercise_id FROM annotations WHERE annotations.id = ' + annotationID + ');';
        ...
        if (!error && response.statusCode == 200) {
          for (var i = 0; i < comparison.systems.length; i++) {
            ...
            for (var j = 0; j < comparison.systems[i].phenotypes.length; j++) {
              for (var k = 0; k < comparison.systems[i].compareToPhenotypes.length; k++) {
                if (comparison.systems[i].phenotypes[j].observed == 1 && comparison.systems[i].compareToPhenotypes[k].observed == 1 && comparison.systems[i].phenotypes[j].hpo == comparison.systems[i].compareToPhenotypes[k].hpo) {
...
app.post('/restricted/class/prof/remove-student', function(req, res) {
  ...
  connection.query(query, function(err, rows) {
    ...
    var query3 = 'DELETE o FROM onset_suggestions o INNER JOIN annotations ON o.annotation_id = annotations.id INNER JOIN exercises ON annotations.exercise_id = exercises.id WHERE class_id = ' + classID + ' AND user_id = ' + userID + ';';
    var query4 = 'DELETE s FROM system_scores s INNER JOIN annotations ON s.annotation_id = annotations.id INNER JOIN exercises ON annotations.exercise_id = exercises.id WHERE class_id = ' + classID + ' AND user_id = ' + userID + ';';
    var query5 = 'DELETE a FROM annotations a INNER JOIN exercises ON a.exercise_id = exercises.id WHERE class_id = ' + classID + ' AND user_id = ' + userID + ';';
    var query6 = 'DELETE FROM students WHERE user_id = ' + userID + ' AND class_id = ' + classID;
    connection.query(query0 + query1 + query2 + query3 + query4 + query5 + query6);
    res.json({ success: true });
  });
});
